...
def stack_vectors(df, column: str) -> np.ndarray:
    """Convert a list-of-lists vector column to a (N, D) float32 array."""
    col = df[column]
...
def stack_embeddings(df) -> np.ndarray:
    ...
    return stack_vectors(df, "embedding")
...
def measure_linear_probes(
    ...
    *,
    ...
    results: Dict[str, Any] = {
        ...
        "probe_fit_cv_n_jobs": int(cv_n_jobs),
    }
    ...
    def build_logreg() -> LogisticRegression:
        ...
        try:
            return LogisticRegression(multi_class=multi_class, **kwargs)
        except TypeError:
            # sklearn >=1.8 may reject the deprecated/removed `multi_class` arg.
            results["probe_fit_multi_class_effective"] = "skipped_for_sklearn_compat"
    ...
    if len(classes) >= 2 and counts.min() >= 5:
        try:
            ...
            auc_scores = cross_val_score(
                ...
                n_jobs=cv_n_jobs,
                pre_dispatch=cv_n_jobs if cv_n_jobs != -1 else "2*n_jobs",
            )
...
def measure_cka_matrix(df, embeddings: np.ndarray, seed: int = 0) -> Dict[str, Any]:
    ...
    # Match sample sizes per role pair. This keeps exploratory CKA comparisons from
    # being unnecessarily throttled by a third role class that is irrelevant
    # to the pair being compared.
    present = [r for r, n in counts.items() if n > 0]
    if not present:
        return {"cka_skipped": "no role data"}
    pair_eligible = [r for r in present if counts[r] >= 5]
    if len(pair_eligible) < 2:
        n_match_min = min(counts[r] for r in present)
        return {"cka_skipped": f"fewer than two role classes have >=5 rows; min class size {n_match_min}", "class_counts": counts}
    n_match_min = min(min(counts[r] for r in pair_eligible), 1000)

    mat: Dict[str, Dict[str, float]] = {}
    n_pair_mat: Dict[str, Dict[str, int]] = {}
    for r1 in present:
        row: Dict[str, float] = {}
        n_row: Dict[str, int] = {}
        for r2 in present:
            n_pair = min(counts[r1], counts[r2], 1000)
            n_row[ROLE_NAMES[r2]] = int(n_pair)
            if r1 == r2:
                row[ROLE_NAMES[r2]] = 1.0
            elif n_pair < 5:
                row[ROLE_NAMES[r2]] = float("nan")
            else:
                x = _subsample(per_role[r1], n_pair, rng)
                y = _subsample(per_role[r2], n_pair, rng)
                row[ROLE_NAMES[r2]] = linear_cka(x, y)
        mat[ROLE_NAMES[r1]] = row
        n_pair_mat[ROLE_NAMES[r1]] = n_row

    out: Dict[str, Any] = {
        "cka_matrix": mat,
        "n_per_role_cka": n_match_min,
        "n_per_pair_cka": n_pair_mat,
    }
...
def run_all(
    ...
    *,
    ...
    if "lstm_h" in df.columns:
        try:
            lstm_h = stack_vectors(df, "lstm_h")
            summary["measurement_1b_lstm_probes"] = measure_linear_probes(
                df,
                lstm_h,
                max_iter=probe_max_iter,
                solver=probe_solver,
            ...
            summary["measurement_1b_lstm_probes"] = {"error": str(exc)}
